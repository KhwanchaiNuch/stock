...
const ReceiptListContainer = ({ hide, key, receiptNo, status }) => {
  ...
  const { onDelete: onCompleteItem } = useDelete('/api/v1/raw-material/receipt-item-complete', true)

  const onCompleteItemFn = async (id, receiptNo) => {
    onCompleteItem({
      variables: {
        receiptItemId: id,
        receiptNo: receiptNo
            ...
            <div className="btn-menu-wrapper">
              {data &&
                ...
                data.result.receipt.status === "NOT_COMPLETE" && (
                  <div
                    className={`btn-menu no-bg ${isEmpty(data.result.receipt.receiptNo) ? "disabled" : ""
                      }`}
                    onClick={() => {
                      openModal({
                        type: "CREATE_RECEIPT",
              ...
              {loadingPrint ? (
                ...
                <div
                  className={`btn-menu ${isEmpty(receiptNo) ? "disabled" : ""
                    }`}
                  onClick={() => {
