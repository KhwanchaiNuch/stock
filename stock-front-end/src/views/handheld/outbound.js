...
const QRScanOutbound = (props) => {
  ...
  // ✅ เคลียร์ค่าฟอร์ม (ไม่ยุ่งกับ recNo) + reset isAllDone
  const clearForm = () => {
  ...
  // 🔹 console log temp ทุกครั้งที่ดึง receipt ใหม่ + เคลียร์ฟอร์มเมื่อ quantity - transactionItemSum = 0 ทุกแถว
  useEffect(() => {
    if (!temp) return
    ...
    onPost({
      ...
      onDone: (res) => {
        ...
        //   })
          ...
          // ไม่ clearForm ตรงนี้ ปล่อยให้ reloadReceipt แล้ว useEffect เป็นตัวเช็ก allDone แทน
          if (refInput.current) {
            refInput.current.focus()
          }
          setLoading(false)
        ...
        <div className="box-scanner">
          <Container>
            ...
            <div className="container-wrapper-section-form">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const limitedValue = Math.min(
                      Number(newValue || 0),
                      inLotQTY >= inPickupQTY ? inPickupQTY : inLotQTY
                    ).toString();
                    setQty(limitedValue);
                  }}
                />
              ...
              {inLotQTY >= inPickupQTY ? '' : (
                <p style={{color : 'red'}}>
                  {'In lot have only ' + inLotQTY}
                </p>
              )}
